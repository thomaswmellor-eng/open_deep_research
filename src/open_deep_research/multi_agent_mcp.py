...
from open_deep_research.utils import get_config_value, tavily_search, duckduckgo_search, get_combined_prompt
from open_deep_research.prompts import SUPERVISOR_INSTRUCTIONS, RESEARCH_INSTRUCTIONS
from open_deep_research.mcp_servers import MCP_SERVERS
...
def make_state_graph(mcp_tools):

    print("mcp_tools")  
    print(mcp_tools)

    # Tool lists will be built dynamically based on configuration
    def get_supervisor_tools(use_mcp=False, config: RunnableConfig=None):
        """Get supervisor tools based on configuration"""
        if use_mcp and mcp_tools:
            tool_list = mcp_tools + [Sections, Introduction, Conclusion]
        else:
    ...
    def get_research_tools(use_mcp=True, config: RunnableConfig=None):
        """Get research tools based on configuration"""
        if use_mcp and mcp_tools:
            tool_list = mcp_tools + [Section]
        else:
            search_tool = get_search_tool(config)
    ...
    async def supervisor(state: ReportState, config: RunnableConfig):
        ...
        supervisor_tool_list, _ = get_supervisor_tools(use_mcp=False, config=config)
    ...
    async def supervisor_tools(state: ReportState, config: RunnableConfig)  -> Command[Literal["supervisor", "research_team", "__end__"]]:
        ...
        # Get tools based on configuration
        _, supervisor_tools_by_name = get_supervisor_tools(use_mcp=False, config=config)
    ...
    async def research_agent(state: SectionState, config: RunnableConfig):
        ...
        # Get tools based on configuration
        research_tool_list, _ = get_research_tools(use_mcp=True, config=config)
        ...
        return {
            "messages": [
                ...
                await llm.bind_tools(research_tool_list).ainvoke(
                    [
                        {"role": "system",
                        "content": RESEARCH_INSTRUCTIONS.format(section_description=state["section"], mcp_prompt=mcp_prompt)
                        }
                    ]
                    + state["messages"]
                )
    ...
    async def research_agent_tools(state: SectionState, config: RunnableConfig):
        ...
        # Get tools based on configuration
        _, research_tools_by_name = get_research_tools(use_mcp=True, config=config)
...
async def make_graph():
    """Create a graph with MCP tools for web research.
    
    This function sets up a connection to specified MCP servers,
    loads the available tools, and yields the graph.
    
    Returns:
        A context manager that yields the graph
    """
        
    # MCP servers to use
    server_names=["filesystem"]

    # Build the server configuration dictionary for MultiServerMCPClient
    server_config = {name: MCP_SERVERS[name] for name in server_names if name in MCP_SERVERS}
    
    # Create combined prompt for the selected servers
    global mcp_prompt
    mcp_prompt = get_combined_prompt(server_names)
    ...
    async with MultiServerMCPClient(server_config) as client:
        ...
        # Compile the graph
        graph = make_state_graph(mcp_tools)
