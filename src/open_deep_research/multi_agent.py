...
async def supervisor_tools(
    ...
    if sections_list:
        ...
        return Command(
            goto="supervisor",
            update={"final_report": intro_content, "messages": result},
        )
        ...
        return Command(
            goto="supervisor",
            update={"final_report": complete_report, "messages": result},
        )
    else:
...
async def supervisor_should_continue(
    state: ReportState,
) -> Literal["supervisor_tools", END]:
...
async def research_agent(state: SectionState, config: RunnableConfig):
    ...
    target_messages = [
        {
            "role": "system",
            "content": RESEARCH_INSTRUCTIONS.format(
                section_description=state["section"]
            ),
        }
    ] + (
        state["messages"]
        or [{"role": "user", "content": "Write a section for the report."}]
    )

    return {
        "messages": [
            # Enforce tool calling to either perform more search or call the Section tool to write the section
            await llm.bind_tools(research_tool_list).ainvoke(target_messages)
        ]
    }
...
async def research_agent_should_continue(
    state: SectionState,
) -> Literal["research_agent_tools", END]:
    """Decide if we should continue the loop or stop based upon whether the LLM made a tool call"""
...
research_builder = StateGraph(
    SectionState, output=SectionOutputState, config_schema=Configuration
)
